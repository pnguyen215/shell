...
shell::decode::file::aes256cbc() {
    ...
    # Validate input parameters
    if [ -z "$input_file" ] || [ -z "$output_file" ]; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: Missing input or output file" 196 >&2
    ...
    # Check if input file exists
    if [ ! -f "$input_file" ]; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: Input file '$input_file' does not exist" 196 >&2
        return 1
    fi

    # Check if output file already exists
    if [ -e "$output_file" ] && [ "$dry_run" = "false" ]; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: Output file '$output_file' already exists" 196 >&2
        return 1
    fi
    ...
    if [ -z "$key" ]; then
        local hasKey=$(shell::exist_key_conf "SHELL_SHIELD_ENCRYPTION_KEY")
        if [ "$hasKey" = "false" ]; then
            shell::colored_echo "🔴 shell::decode::file::aes256cbc: SHELL_SHIELD_ENCRYPTION_KEY is not set. Please set it or provide a key." 196 >&2
            return 1
        fi
        key=$(shell::get_value_conf "SHELL_SHIELD_ENCRYPTION_KEY")
    fi
    ...
    if [ -z "$iv" ]; then
        local hasIv=$(shell::exist_key_conf "SHELL_SHIELD_ENCRYPTION_IV")
        if [ "$hasIv" = "false" ]; then
            shell::colored_echo "🔴 shell::decode::file::aes256cbc: SHELL_SHIELD_ENCRYPTION_IV is not set. Please set it or provide an IV." 196 >&2
            return 1
        fi
        iv=$(shell::get_value_conf "SHELL_SHIELD_ENCRYPTION_IV")
    fi
    ...
    # Validate key length (64 bytes for AES-256)
    if [ ${#key} -ne 64 ]; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: Encryption key must be exactly 64 bytes" 196 >&2
        return 1
    fi

    # Validate iv length (32 bytes for AES-256)
    if [ ${#iv} -ne 32 ]; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: Initialization vector must be exactly 32 bytes" 196 >&2
        return 1
    fi

    # Check if OpenSSL is installed
    if ! shell::is_command_available openssl; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: OpenSSL is not installed" 196 >&2
        return 1
    fi
    ...
    if ! shell::run_cmd_eval "$cmd" 2>/dev/null; then
        shell::colored_echo "🔴 shell::decode::file::aes256cbc: Decryption failed. Please check your key and try again." 196 >&2
        return 1
    fi
