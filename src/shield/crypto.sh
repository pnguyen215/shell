...
# shell::cryptography::verify_password_hash function
# Verifies a plain text password against an OpenSSL-generated password hash.
#
# Usage:
#   shell::cryptography::verify_password_hash [-h] <password> <hashed_password>
#
# Parameters:
#   - -h             : Optional. Displays this help message.
#   - <password>     : The plain text password to verify.
#   - <hashed_password>: The hash string generated by `openssl passwd`.
#
# Description:
#   This function uses `openssl passwd` with the `-check` option to verify if a
#   given plain text password matches a previously generated hash. This is the
#   standard way to check user passwords stored as hashes without storing the
#   plain text.
#
# Example:
#   # Assume 'my_hashed_pass' holds a hash like '$2a$08$abcdefghijklmnopqrstuu.V1E.qfFm/9Wc0bF3x7k7H/sCq7d.uA/'.
#   if shell::cryptography::verify_password_hash "MySecurePassword123!" "$my_hashed_pass"; then
...
#
# Returns:
#   0 on successful verification (password matches hash), 1 on failure (password does not match, or error).
#
# Notes:
#   - Requires OpenSSL to be installed.
#   - The `hashed_password` must include the algorithm identifier and salt.
shell::cryptography::verify_password_hash() {
    # Check for the help flag (-h)
    if [ "$1" = "-h" ]; then
        echo "$USAGE_SHELL_CRYPTOGRAPHY_VERIFY_PASSWORD_HASH"
        return 0
    fi
    ...
    if [ -z "$password" ] || [ -z "$hashed_password" ]; then
        shell::colored_echo "🔴 shell::cryptography::verify_password_hash: Missing required parameters." 196 >&2
        echo "Usage: shell::cryptography::verify_password_hash [-h] <password> <hashed_password>"
        return 1
    fi

    if ! shell::is_command_available openssl; then
        shell::colored_echo "🔴 shell::cryptography::verify_password_hash: OpenSSL is not installed." 196 >&2
        return 1
    fi

    # Use openssl passwd -check. It expects the hash as the first argument,
    # and the password from stdin. It exits with 0 on success, non-zero on failure.
    printf "%s" "$password" | openssl passwd -check "$hashed_password" 2>/dev/null
