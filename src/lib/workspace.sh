...
shell::add_workspace() {
	...
	# If dry_run is true, we call shell::logger::copy with the command
...
shell::add_workspace_ssh_conf() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::copy "touch \"$file\" && shell::populate_ssh_conf \"$file\" \"$conf\""
	else
...
shell::remove_workspace() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::copy "sudo rm -rf \"$dir\""
		return 0
	fi
	local asked
