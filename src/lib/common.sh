...
#
# Usage:
#   shell::exec::safe <dry_run> <command>
#
# Parameters:
...
shell::exec::safe() {
	if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
		...
		shell::logger::usage "shell::exec::safe [-h | --help] <dry_run> <command>"
		...
		shell::logger::example "shell::exec::safe true echo Hello"
		shell::logger::example "shell::exec::safe false echo Hello"
		return $RETURN_SUCCESS
	fi
...
#
# Usage:
#   shell::file::mime [-h] [--help] <file_path>
#
# Parameters:
...
# Returns:
#   $RETURN_SUCCESS (0) on success.
#   $RETURN_INVALID (1) when the file path is missing or invalid.
#   $RETURN_FAILURE (non-zero) when the target file does not exist.
#
# Example:
