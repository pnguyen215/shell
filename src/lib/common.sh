...
shell::clip_value() {
	...
	if [[ "$os" == "macos" ]]; then
		echo -n "$value" | pbcopy
		shell::logger::debug "Value copied to clipboard using pbcopy."
		return 1
	elif [[ "$os" == "linux" ]]; then
		if shell::is_command_available xclip; then
			echo -n "$value" | xclip -selection clipboard
			shell::logger::debug "Value copied to clipboard using xclip."
			return 1
		elif shell::is_command_available xsel; then
			echo -n "$value" | xsel --clipboard --input
			shell::logger::debug "Value copied to clipboard using xsel."
			return 1
		else
			shell::logger::error "Clipboard tool not found. Please install xclip or xsel."
			return 0
...
shell::download_dataset() {
	...
	if [ -e "$base" ]; then
		...
		if [ "$asked" = "yes" ]; then
			if [ "$dry_run" = "true" ]; then
				shell::logger::cmd_copy "sudo rm \"$base\""
			else
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::cmd_copy "$download_cmd"
		return 0
	else
...
shell::loading_spinner() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::cmd_copy "Display loading spinner for ${duration} seconds"
		return 0
	fi
...
shell::unlock_permissions() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::cmd_copy "$chmod_cmd"
	else
...
shell::fzf_set_permissions() {
	...
	# If dry_run is true, the command is printed using shell::logger::cmd_copy.
...
shell::encode_base64_file() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::cmd_copy "$base64_cmd"
	else
