...
shell::kill_port() {
	...
	for port in "$@"; do
		...
		if [ -n "$pids" ]; then
			...
			for pid in $pids; do
				# Construct the kill command as an array to reuse it for both shell::logger::copy and shell::run_cmd.
...
#   - Helper functions: shell::run_cmd, shell::logger::copy, shell::stdout, and shell::get_os_type.
shell::editor() {
	if [ "$1" = "-h" ]; then
...
shell::download_dataset() {
	...
	if [ -e "$base" ]; then
		...
		if [ "$asked" = "yes" ]; then
			if [ "$dry_run" = "true" ]; then
				shell::logger::copy "sudo rm \"$base\""
			else
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::copy "$download_cmd"
		return 0
	else
...
shell::loading_spinner() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::copy "Display loading spinner for ${duration} seconds"
		return 0
	fi
...
shell::fzf_set_permissions() {
	...
	# If dry_run is true, the command is printed using shell::logger::copy.
...
# shell::parse_options function
# Parses command-line options for shell utility functions and scripts.
#
# Usage:
#   shell::parse_options args_var verbose_var dry_run_var force_var help_var -- "$@"
#
# Parameters:
#   - args_var    : Name ref to an array variable set to remaining non-option arguments.
#   - verbose_var : Name ref to variable set to "true" if verbose mode enabled, else "false".
#   - dry_run_var : Name ref to variable set to "true" if dry-run, else "false".
#   - force_var   : Name ref to variable set to "true" if force mode, else "false".
#   - help_var    : Name ref to variable set to "true" if help mode, else "false".
#   - --          : Separator before passing command-line arguments to parse.
...
#   Grouped short options (e.g., -vnf) are supported.
#
# Output:
#   Sets the referenced variables based on detected options.
#   Populates args_var with the remaining non-option arguments.
#
# Description:
#   This function provides a generic, reusable way to parse common CLI options for shell utility functions.
#   It supports both short and long options, as well as grouped short options.
#   All option flags are set to "false" by default and will be switched to "true" if present in the arguments.
