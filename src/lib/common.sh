...
#
# Parameters:
#   - -a         : (Optional) Include hidden files (files starting with a dot).
...
#     - Detailed privileges (permissions) in color
#     - Owner (user)
#     - Created at timestamp
#     - Modified at timestamp in color
#     - File/directory name
#
...
# Example:
#   list
#   list -a /path/to/directory
list() {
    local include_hidden="false"
    # Check if first argument is the "-a" flag to include hidden files
    if [ "$1" = "-a" ]; then
        include_hidden="true"
        shift
    fi
    ...
    # Print header with aligned columns: Privileges, Owner, Created At, Modified At, Name
    printf "%-12s %-12s %-25s %-25s %-s\n" "Privileges" "Owner" "Created At" "Modified At" "Name"
    printf "%-12s %-12s %-25s %-25s %-s\n" "----------" "-----" "----------" "-----------" "----"

    # Determine file pattern based on include_hidden flag.
    local pattern=()
    ...
    # Loop over each file/directory matching the pattern
    for entry in "${pattern[@]}"; do
        # Skip if no file is found or if entry is not valid
        [ -e "$entry" ] || continue

        local details=""
        if [ "$os_type" = "macos" ]; then
            # macOS: Use stat with format: permissions, owner, creation time, modification time
            details=$(stat -f "%Sp;%Su;%SB;%Sm" "$entry" 2>/dev/null)
        else
            # Linux: Use stat with format: permissions, owner, creation time, modification time
            details=$(stat -c "%A;%U;%w;%y" "$entry" 2>/dev/null)
        fi

        # Check if details were retrieved successfully
        if [ $? -ne 0 ]; then
            colored_echo "🔴 Error retrieving details for '$entry'" 196
            continue
        fi
        ...
        # Use colored_echo to produce colored output for privileges and modified-at columns.
        # Capture the colored output in variables.
        local col_priv col_modified
        col_priv=$(colored_echo "$privileges" 33)
        col_modified=$(colored_echo "$modified" 46)

        # Print the file details in aligned columns including the file/directory name.
        printf "%-12s %-12s %-25s %-25s %-s\n" "$col_priv" "$owner" "$created" "$col_modified" "$(basename "$entry")"
    done
}
