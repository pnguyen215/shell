...
shell::kill_port() {
	...
	for port in "$@"; do
		...
		if [ -n "$pids" ]; then
			...
			for pid in $pids; do
				# Construct the kill command as an array to reuse it for both shell::logger::command_clip and shell::run_cmd.
...
shell::loading_spinner() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::command_clip "Display loading spinner for ${duration} seconds"
		return 0
	fi
...
shell::unlock_permissions() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::command_clip "$chmod_cmd"
	else
...
shell::fzf_set_permissions() {
	...
	# If dry_run is true, the command is printed using shell::logger::command_clip.
