...
shell::read_conf() {
	...
	if [ "$dry_run" = "true" ]; then
		...
		shell::exec::command source "$filename"
	fi
}
...
shell::sync_group_key_conf() {
	...
	if [ "$dry_run" = "true" ]; then
		...
		shell::exec::shell "sudo rm $temp_file"
	else
		...
		shell::exec::shell "sudo cp $SHELL_GROUP_CONF_FILE $backup_file"
		shell::exec::shell "sudo mv $temp_file $SHELL_GROUP_CONF_FILE"
...
shell::add_protected_key_conf() {
	...
	# Use shell::logger::command_clip for dry-run mode, otherwise run the command.
...
shell::sync_protected_key_conf() {
	...
	if [ "$dry_run" = "true" ]; then
		...
		shell::exec::shell "sudo mv \"$temp_file\" \"$protected_file\""
