...
shell::read_conf() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::copy "source \"$filename\""
	else
...
#   - Helper functions: shell::install_package, shell::stdout, shell::get_os_type, shell::clip_value, shell::logger::copy.
#
# Example usage:
#   shell::fzf_view_key_conf_viz         # Select a key and copy its decoded value.
#   shell::fzf_view_key_conf_viz -n      # Dry-run: print the clipboard copy command.
#
# Returns:
...
shell::add_protected_key_conf() {
	...
	# Use shell::logger::copy for dry-run mode, otherwise run the command.
