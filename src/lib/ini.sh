...
shell::fzf_edit_ini_viz() {
	...
	shell::colored_echo "1) Edit Value" 244
	shell::colored_echo "2) Remove Key" 244
	shell::colored_echo "3) Remove Section" 244
	shell::colored_echo "4) Rename Section" 244
	shell::colored_echo "5) Add Section" 244
	shell::colored_echo "6) Add Key" 244
	shell::colored_echo "7) Cancel" 244

	local choice
	choice=$(shell::enter "Enter your choice (1-7):")
	...
	2)
		local confirm
		confirm=$(shell::enter "Are you sure you want to remove the key '$key' from section [$section]? (y/n):")
		if [[ "$confirm" =~ ^[Yy]([Ee][Ss])?$ ]]; then
			shell::colored_echo "DEBUG: Removing key '$key' from section [$section] in file '$file'..." 244
			shell::remove_ini_key "$file" "$section" "$key"
			return $?
		else
			shell::colored_echo "WARN: Operation cancelled." 11
			return 0
		fi
		;;
	3)
		local confirm
		confirm=$(shell::enter "Are you sure you want to remove the section [$section]? (y/n):")
		if [[ "$confirm" =~ ^[Yy]([Ee][Ss])?$ ]]; then
			shell::colored_echo "DEBUG: Removing section [$section] from file '$file'..." 244
			shell::remove_ini_section "$file" "$section"
			return $?
	...
	4)
		local confirm
		confirm=$(shell::enter "Are you sure you want to rename the section [$section]? (y/n):")
		if [[ "$confirm" =~ ^[Yy]([Ee][Ss])?$ ]]; then
	...
	7)
		...
		;;
	*)
		shell::colored_echo "ERR: Invalid option '$choice'. Please choose 1-7." 196
		return 1
		;;
	esac
