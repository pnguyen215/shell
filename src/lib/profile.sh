...
shell::remove_profile() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::command_clip "sudo rm -rf \"$profile_dir\""
	else
		shell::stdout "Are you sure you want to remove profile '$profile_name'? [y/N]" 33
		read -r confirm
		if [[ "$confirm" =~ ^[Yy]$ ]]; then
			shell::exec::shell sudo rm -rf "$profile_dir"
...
shell::get_profile() {
	...
	shell::exec::shell cat "$profile_conf"
}
...
shell::add_profile_conf() {
	...
	if [ ! -d "$profile_dir" ]; then
		if [ "$dry_run" = "true" ]; then
			...
			shell::exec::shell sudo mkdir -p "$profile_dir"
		fi
	fi
