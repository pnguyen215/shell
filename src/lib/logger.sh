...
# Logs and executes a shell command safely with options and enhanced logger integration.
#
# Usage:
#   shell::logger::exec_safe [-v|--verbose] [-n|--dry-run] [-f|--force] [-h|--help] <command> [args...]
#
# Options:
...
#
# Description:
#   This function parses common shell CLI options, logs the command, checks for dangerous usage (such as rm -rf),
#   and executes the command if allowed by the flags. Uses logger functions for all output, warnings, and errors.
shell::logger::exec_safe() {
    ...
    if [[ "$help" == true ]]; then
        shell::logger::reset_options
        shell::logger::usage "shell::logger::exec_safe [OPTIONS] <command> [args...]"
        shell::logger::option "-v, --verbose"    "Enable verbose output (set -x)"
        shell::logger::option "-n, --dry-run"    "Show the command, do not execute"
        shell::logger::option "-f, --force"      "Force execution of dangerous commands"
        shell::logger::option "-h, --help"       "Show this help message"
        shell::logger::example "shell::logger::exec_safe -vnf echo Hello"
        shell::logger::example "shell::logger::exec_safe --force rm -rf /tmp/test"
        return 0
    fi

    if [[ "${#args[@]}" -eq 0 ]]; then
        shell::logger::error "No command specified. Use --help for usage."
        return 1
    fi
    ...
    elif [[ "${args[0]}" == "sudo" && "${args[1]}" == "rm" ]]; then
    ...
    if [[ $dangerous -eq 1 && "$force" != true ]]; then
		shell::logger::warn "Dangerous command detected: ${args[*]}"
        shell::logger::warn "Use --force to allow execution."
        return 1
    fi

    shell::logger::cmd "${args[*]}"
    if [[ "$dry_run" == true ]]; then
        return 0
    fi

    if [[ "$verbose" == true ]]; then
        shell::logger::debug "Enabling verbose mode (set -x)"
        set -x
    fi
...
# Executes a command safely via shell::logger::exec_safe, then prints a custom success or failure message.
#
# Usage:
#   shell::logger::exec_safe_check [OPTIONS] <command> [args...] [<success_msg> <fail_msg>]
#
# Description:
#   This function reuses shell::logger::exec_safe to execute the command with all options and safety checks.
#   If two trailing arguments are present, they are treated as the success and failure messages.
#   The result is logged via shell::logger::success or shell::logger::error.
shell::logger::exec_safe_check() {
    local args=("$@")
    local msg_success="Success"
    local msg_fail="Aborted"
    local cmd_args=()

	# Extract success/fail message if supplied
    if (( ${#args[@]} >= 3 )); then
        msg_success="${args[-2]}"
        msg_fail="${args[-1]}"
    ...
    shell::logger::exec_safe "${cmd_args[@]}"
    local status=$?

    if [[ $status -eq 0 ]]; then
		shell::colored_echo "  [✓] $msg_success" 46
    else
		shell::colored_echo "  [✗] $msg_fail (exit code: $status)" 196
    fi
    return $status
}