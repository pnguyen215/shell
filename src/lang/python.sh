...
shell::python::pip::uninstall::all() {
	...
	if [ "$dry_run" = "false" ]; then
		local asked=$(shell::ask::reply "Are you absolutely sure you want to proceed?")
		if [ "$asked" = "no" ]; then
...
shell::python::venv::create() {
	...
	if [ "$dry_run" = "false" ]; then
		if [ -d "$venv_path" ]; then
			...
			local ask=$(shell::ask::reply "Do you want to overwrite the existing virtual environment?")
			if [ "$ask" = "yes" ]; then
...
shell::python::venv::pkg::uninstall() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::command_clip "$uninstall_cmd"
		return $RETURN_SUCCESS
	fi
...
shell::python::venv::activate::fzf() {
	...
	if [ -n "$VIRTUAL_ENV" ]; then
		local ask=$(shell::ask::reply "Do you want to deactivate it first?")
		if [ "$ask" = "yes" ]; then
...
shell::python::venv::pkg::freeze() {
	...
	if [ "$dry_run" = "true" ]; then
		shell::logger::command_clip "$freeze_cmd"
		return $RETURN_SUCCESS
	fi
